# Based on nixpkgs ec2e31469a704695b8aeceacf22a183098e5f7d4 pkgs/build-support/setup-hooks/patch-shebangs.sh
...
patchShebangs() {
    ...
    while IFS= read -r -d $'\0' f; do
        ...
        # Compute a ${ORIGIN}-relative path from the script to the interpreter,
        # so the package is relocatable as long as store-relative layout is preserved.
        local relativePath
        if [[ -n "$newPath" ]]; then
            local scriptDir
            scriptDir="$(dirname "$f")"
            relativePath="\${ORIGIN}/$(realpath --no-symlinks --relative-to="$scriptDir" "$newPath")"
        fi

        # Strip trailing whitespace introduced when no arguments are present
        newInterpreterLine="${relativePath:-$newPath} $args"
